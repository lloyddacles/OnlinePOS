...
router.post("/", requireAuth, (req, res) => {
  const { name, type, price = 0, sort_order = 0 } = req.body;
  if (!name || !["sweetness", "ice", "topping"].includes(type)) {
    return res.status(400).json({ error: "Valid name and type required (sweetness|ice|topping)" });
  }
  const result = db
    .prepare("INSERT INTO addons (name, type, price, sort_order) VALUES (?, ?, ?, ?)")
    .run(name.trim(), type, Number(price) || 0, Number(sort_order) || 0);
...
router.put("/:id", requireAuth, (req, res) => {
  ...
  const { name, type, price, sort_order, available } = req.body;
  db.prepare(
    "UPDATE addons SET name = ?, type = ?, price = ?, sort_order = ?, available = ? WHERE id = ?"
  ).run(
    name ?? existing.name,
    type ?? existing.type,
